...
TLaserDiodeDriverOutputParams = collections.namedtuple("TLaserDiodeDriverOutputParams",
                                                       ["power", "current", "voltage"])
TLDLaserDiodeDriverStatus = collections.namedtuple("TLDLaserDiodeDriverStatus",
                                                   ["laser_current", "photo_current", "laser_voltage",
                                                    "output", "keyswitch", "ctl_mode", "interlock", "tia_range",
                                                    "polarity", "sma_enabled", "open_circuit", "psu_ok",
                                                    "tia_overlimit", "tia_underlimit",
...
class KinesisLaserDiodeDriver(BasicKinesisDevice):
    ...
    def _la_req(self, subid):
        ...
        if rsubid != subid:
            raise RuntimeError(
                "unexpected submessage ID in the reply: expected 0x{:02x}, got 0x{:02x}".format(subid, rsubid))
        return data[2:]
    ...
    _status_bits = {"output": 0, "keyswitch": 1, "ctl_mode": 2, "interlock": 3, "polarity": 8, "sma_enabled": 9,
                    "open_circuit": 11, "psu_ok": 12, "tia_overlimit": 13, "tia_underlimit": 14,
    ...
    def get_laser_generic(self, input):
        """Get whatever -- generic command for debug of get_laser_output_parameters subid problem"""
        data = self._la_req(input)
        return data

    def enable_max_current_adjust(self, enabled: bool = True, enable_while_laser_on: bool = False):
        """Enable or disable the ability to adjust the maximum current"""
        self.send_comm(0x0816, 0x02 if enabled else 0x01, 0x02 if enable_while_laser_on else 0x01)

    def set_max_current(self, value):
        """Sets the max current (in percent), which gives 0 to 200mA for the TLD001 and 230mA for the KLD101"""
        ivalue = max(0, min(int(value / 100 * 255), 255))
        self.send_comm(0x0817, ivalue)

    def get_max_current(self):
        """Returns the max current (in percent), which represents 0 to 200mA for the TLD001 and 230mA for the KLD101"""
        value = self.query(0x0818).param1
        return value / 255 * 100
        # Alternative:
        # data = self._la_req(0x0a)
        # return struct.unpack("<H", data)[0] / 32767 * 100
